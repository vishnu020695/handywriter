...
# --- Tab Layout Defined Cleanly Here ---
tab1, tab2, tab3 = st.tabs([
    "📝 Image → Word / Excel", 
    "📄 Intelligent Single PDF Editor", 
...
# ---------------------------------------------------------------------------
# TAB 2: Intelligent Single PDF Editor
# ---------------------------------------------------------------------------
with tab2:
    st.subheader("🔍 Intelligent Search & Replace (Best for Single Multi-page Offer Letters)")
    ...
    if uploaded_pdf:
        ...
        with col_btn1:
            if st.button("➕ Add More Fields", key="add_field_btn"):
                st.session_state.replacements_count += 1
                st.rerun()
        with col_btn2:
            if st.button("🗑️ Remove Last Field", key="remove_field_btn") and st.session_state.replacements_count > 1:
                st.session_state.replacements_count -= 1
                st.rerun()
        ...
        if st.button("🚀 Apply Changes and Download PDF", use_container_width=True, key="apply_single_pdf"):
            if not change_dict:
